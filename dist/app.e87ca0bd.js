...
$(function () {
  ...
  $(window).scroll(function () {
    // Declare vars
    var offset = 100;
    var scrolling = $(this).scrollTop(); // Get document entire height
    ...
    var pieAxisScroll = windowHeight / (this.Math.PI * scrolling); // scrolling behaviour logo / menu
    ...
    if ($('.pane').height() !== this.undefined) {
      ...
      if (formulaA > 0.5 && scrolling < last4ScrollPane) {
        $('.work--pane').css('transform', 'translateY(' + pieAxisScroll * 0.25 + 'px)');
        $('h2.work--title').css('transform', 'matrix( 1, 0, 0, 1, 0, ' + formulaB + ')'); // animate in gsap

        gsap.to('.w--pane--sports', {
          opacity: 1,
          duration: 2,
          delay: 1.2
        });
